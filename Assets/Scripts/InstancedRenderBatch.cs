...
/// </summary>
public class InstancedRenderBatch<T> : IDisposable where T : struct
{
    ...
    /// </summary>
    public void UpdateData(NativeArray<T> data)
    {
        ...
        if (_dataBuffer == null || _dataBuffer.count != count)
        {
            ...
            if (count > 0)
            {
                ...
                _dataBuffer = new ComputeBuffer(count, Unity.Collections.LowLevel.Unsafe.UnsafeUtility.SizeOf<T>());
            }
        }
    ...
    public void Draw()
    {
        if (_dataBuffer == null || _argsBuffer == null || Mesh == null || Material == null || _dataBuffer.count == 0) return;
